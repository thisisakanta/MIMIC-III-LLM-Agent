...
MAX_HOURS = 48.0
DT_HOURS = 1.0
T_STEPS = int(MAX_HOURS / DT_HOURS)
...
print(f"Running GRU-D Experiment 2 subset: {EXP2_SUBSET}")
...
def compute_train_stats_from_matrices(
    ...
    for stay in train_list["stay"].tolist():
        ...
        x = np.load(x_path).astype(np.float32)   # [T, 17]
        m = np.load(m_path).astype(np.float32)   # [T, 17]
...
class IHM48hSubsetDataset(Dataset):
    def __init__(
        self,
    ...
    def _load_one(self, idx: int) -> Sample:
        ...
        x_raw = np.load(x_path).astype(np.float32)   # [T, 17]
        m = np.load(m_path).astype(np.float32)       # [T, 17]

        x_raw = x_raw[:, self.subset_indices]
...
def main():
    ...
    mean, std = compute_train_stats_from_matrices(
        train_df,
        MATRIX_DIR["train"],
        SUBSET_INDICES,
    )
    emp_mean = compute_empirical_mean_normalized(len(SUBSET_INDICES))

    print(f"Computed train normalization stats for subset={EXP2_SUBSET}")
    print(f"Subset feature count: {len(SUBSET_INDICES)}")

    cache_dir = DATA_DIR / "cache_grud_exp2_subsets"
    cache_dir.mkdir(parents=True, exist_ok=True)

    train_cache = cache_dir / f"train_cache_exp2_{EXP2_SUBSET}.pt"
    val_cache = cache_dir / f"val_cache_exp2_{EXP2_SUBSET}.pt"
    test_cache = cache_dir / f"test_cache_exp2_{EXP2_SUBSET}.pt"

    train_set = IHM48hSubsetDataset(
        listfile=train_df,
    ...
    best_val_pr = -1.0
    best_path = cache_dir / f"grud_best_model_exp2_{EXP2_SUBSET}.pt"
    no_improve = 0

    print(f"\nTraining GRU-D Experiment 2 subset={EXP2_SUBSET}...\n")
    ...
    print("\nSaving GRU-D Experiment 2 prediction files...")

    val_pred_df = collect_predictions(model, val_loader, DEVICE)
    test_pred_df = collect_predictions(model, test_loader, DEVICE)

    val_path = cache_dir / f"grud_val_predictions_exp2_{EXP2_SUBSET}.csv"
    test_path = cache_dir / f"grud_test_predictions_exp2_{EXP2_SUBSET}.csv"
